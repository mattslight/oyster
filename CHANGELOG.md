...
- **Right-click a space pill for Rename · Color · Delete.** The context menu that used to live on the (now-removed) chat-bar pills moved to the breadcrumb. *Delete* opens a confirm that names how many sessions are inside the targeted space and that they'll return to Elsewhere. Works on any non-meta pill, regardless of whether the space has folders attached.
