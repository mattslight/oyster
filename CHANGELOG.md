...
- `onboard_space` now treats `paths` as optional — creating a space without any filesystem attachment leaves it as a logical grouping (summaries, memories, and artifacts can attach later). Useful for importing conceptual spaces from another AI where the user hasn't yet pointed at folders on disk.
