...
- **Setup chip moves next to "+ New session".** The Oyster setup affordance now sits inline in the home breadcrumb as its own chip — separate from the *1 running · + New session* cluster, since setup isn't part of the session-flow group. The mid-progress state ("required step done, optionals pending") gets a gold *◐* half-circle glyph in place of the green check — the chip body stays purple so it doesn't out-shout the active space pill, and the glyph alone signals halfway-through. When everything's done, the chip becomes a tiny green pill with a 🦪 oyster — your "found the pearl" moment.
