...
- **Right-click works on the artefacts list view.** Pin / Unpin, Edit share, Unpublish, and Publish… now appear on a context menu in the table view too — previously the browser default menu took over.
- **Manage cloud-only publications from any signed-in device.** Right-click an `On cloud` tile (or list-view row) for **Edit share…** and **Unpublish**. Edit-share opens the regular publish modal so you can change mode, set/clear/reset the password, or switch to sign-in-required — without needing the original artefact bytes on this device.
