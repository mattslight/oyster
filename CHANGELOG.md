...
- **Pin artefacts to the top.** Right-click any artefact for `Pin to top`; pinned items sort above folder tiles and other artefacts in the active scope, most-recent pin first. Right-click again for `Unpin`. Filters still apply — pinning doesn't override filter visibility. ([#387](https://github.com/mattslight/oyster/issues/387))
